...
def run_C02_ex08(student_file):
    ...
    for i, s in enumerate(test_cases, 1):
        expected = expected_lowcase(s)
        actual, err, code = run_test_case(exe_path, input_data=s)
        desc = f"ft_strlowcase(str={display_str(s)})"
    ...
    print_exercise_result("ex08/ft_strlowcase.c", passed_count, total_count)
