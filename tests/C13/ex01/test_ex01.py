...
def run_C13_ex01(student_file):
    ...
    for i, values in enumerate(test_cases, 1):
        expected = prefix(build_tree(values))
        actual, err, code = run_test_case(exe_path, input_data=wire(values))
        desc = f"btree_apply_prefix(tree={values})"
    ...
    print_exercise_result("ex01/btree_apply_prefix.c", passed_count, total_count)
