...
def run_C01_ex03(student_file):
    ...
    for i, (a, b, expected) in enumerate(test_cases, 1):
        ...
        if actual == expected:
            print_test_pass(i, f"{desc}: div and mod set to {actual} as expected")
            passed_count += 1
        else:
            print_test_fail(
                i,
                f"{desc}: set div and mod not as expected",
                expected=expected,
                actual=actual,
            )

    for flag, label in [("null_div", "div=NULL"), ("null_mod", "mod=NULL")]:
        total_count += 1
