...
def run_C12_ex09(student_file):
    ...
    test_cases = [[], [1], [1, 2, 3], list(range(10))]
    ...
    for i, tab in enumerate(test_cases, 1):
        ...
        desc = f"ft_list_foreach(list of {tab}, f=record)"
    ...
    if err == "TIMEOUT":
        print_test_fail(total_count, "f=NULL on a non-empty list: hung (timed out)")
    elif code < 0:
        print_test_fail(
            total_count, "f=NULL on a non-empty list: crashed", actual=f"signal {-code}"
        )
    else:
        print_test_pass(total_count, "f=NULL on a non-empty list: did not crash or hang")
        passed_count += 1

    print_exercise_result("ex09/ft_list_foreach.c", passed_count, total_count)
