...
def run_C02_ex03(student_file):
    ...
    for i, s in enumerate(test_cases, 1):
        expected = expected_is_numeric(s)
        actual, err, code = run_test_case(exe_path, input_data=s)
        desc = f"ft_str_is_numeric(str={display_str(s)})"
    ...
    print_exercise_result("ex03/ft_str_is_numeric.c", passed_count, total_count)
