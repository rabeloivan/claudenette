...
def run_C11_ex03(student_file):
    ...
    for i, tab in enumerate(test_cases, 1):
        expected = sum(1 for s in tab if has_digit(s))
        input_str = f"{len(tab)}\n" + "\n".join(tab)
        actual, err, code = run_test_case(exe_path, input_data=input_str)
        desc = f"ft_count_if(tab={tab}, length={len(tab)}, f=has_digit)"
    ...
    print_exercise_result("ex03/ft_count_if.c", passed_count, total_count)
