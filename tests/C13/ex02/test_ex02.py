...
def run_C13_ex02(student_file):
    ...
    for i, values in enumerate(test_cases, 1):
        expected = infix(build_tree(values))
        actual, err, code = run_test_case(exe_path, input_data=wire(values))
        desc = f"btree_apply_infix(tree={values})"
    ...
    print_exercise_result("ex02/btree_apply_infix.c", passed_count, total_count)
