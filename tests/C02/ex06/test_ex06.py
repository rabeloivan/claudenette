...
def run_C02_ex06(student_file):
    ...
    for i, s in enumerate(test_cases, 1):
        expected = expected_is_printable(s)
        actual, err, code = run_test_case(exe_path, input_data=s)
        desc = f"ft_str_is_printable(str={display_str(s)})"
    ...
    print_exercise_result("ex06/ft_str_is_printable.c", passed_count, total_count)
