...
def run_C00_ex01(student_file):
    ...
    expected = "abcdefghijklmnopqrstuvwxyz" * 2
    actual, err, code = run_test_case(exe_path, input_data="")
    passed = False

    if actual == expected:
        print_test_pass(1, f"ft_print_alphabet() called twice: output {actual} as expected")
        passed = True
    else:
        print_test_fail(
            1,
            "ft_print_alphabet() called twice: output not as expected",
            expected=expected,
            actual=actual,
        )
